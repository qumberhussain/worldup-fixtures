...
components/
  ...
  LiveRefresher.tsx       Client: router.refresh() on interval (live match pages only)
lib/
...
10. **Per-match pages.** Added `/match/[slug]` (SSG via `generateStaticParams` +
    ISR) — the biggest long-tail SEO multiplier (1 page → 100+). Pretty slug
    `home-vs-away-<id>` that canonical-redirects on slug drift; per-match
    `generateMetadata`, **server-rendered** goals/cards timeline (was JS-only),
...
- ✅ **Per-match pages** (`/match/[slug]`, SSG/ISR) — **done**. Pretty slug
  (`brazil-vs-morocco-3`, canonical-redirects on slug drift), per-match
  `generateMetadata`, server-rendered goals/cards timeline (now crawlable),
  `SportsEvent`+`BreadcrumbList` JSON-LD, per-match OG card, sitemap entry per
  match, and live pages self-refresh via `router.refresh()`. *Next:* per-match
  text previews/reports for ranking depth.
...
- **Shareable match cards** (OG images) and deep links to a single match — ✅ done:
  site-level + per-match OG cards, and every match has its own `/match/[slug]` URL.
