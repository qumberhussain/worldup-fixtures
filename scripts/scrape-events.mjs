...
function parsePage(wt) {
  const out = {};
  const re =
    /team1=\{\{#invoke:flag\|fb-rt\|([A-Z]{3})\}\}[\s\S]*?team2=\{\{#invoke:flag\|fb\|([A-Z]{3})\}\}([\s\S]*?)\|stadium=/g;
  let m;
  while ((m = re.exec(wt))) {
    const [, t1, t2, tail] = m;
    const idx = tail.indexOf("|goals2=");
    if (idx < 0) continue;
    const goals1 = tail.slice(0, idx).replace(/^[\s\S]*?\|goals1=/, "");
    const goals2 = tail.slice(idx + "|goals2=".length);
    const goals = [...parseGoals(goals1, t1), ...parseGoals(goals2, t2)];
    if (goals.length) out[`${t1}|${t2}`] = { goals };
  }
  return out;
}
